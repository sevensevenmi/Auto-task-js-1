...
for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (cookiesArr[i]) {
      ...
      $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])
      if ($.canHelp) {
        if ($.tuanList.length) console.log(`开始账号内部互助 赚京豆-瓜分京豆`)
        for (let j = 0; j < $.tuanList.length; ++j) {
          console.log(`账号 ${$.UserName} 开始给 【${$.tuanList[j]['assistStartRecordId']}】助力`)
